...
class Tracker:
    ...
    max_age : int
        ...
        一个跟踪对象丢失多少帧后会被删去（删去之后将不再进行特征匹配）也是级联特征匹配中的最大深度，比如说该值设为10，我现在是第12帧，那么第一帧的数据就不在进行特征匹配了。
    n_init : int
    ...
    def __init__(self, metric, max_iou_distance=0.7, max_age=70, n_init=3):
        self.metric = metric
        self.max_iou_distance = max_iou_distance  # 做IOU匹配的时候用到的阈值
    ...
    def _initiate_track(self, detection):  # 初始化一个新的跟踪对象

...
if __name__ == '__main__':
    image1 = cv2.imread("img/222.jpg")
    image1 = cv2.cvtColor(image1, cv2.COLOR_BGR2RGB)[np.newaxis, :, :]
    image2 = cv2.imread("img/333.jpg")
    image2 = cv2.cvtColor(image2, cv2.COLOR_BGR2RGB)[np.newaxis, :, :]
    extr = Extractor("checkpoint/ckpt.t7")
    feature1 = extr(image1)
    print(feature1.shape)
    feature2 = extr(image2)
    metric = NearestNeighborDistanceMetric("cosine", 0.2)
    cost = metric.cost_distance([feature1], [feature2])
    print(cost)
    print(cost.shape)

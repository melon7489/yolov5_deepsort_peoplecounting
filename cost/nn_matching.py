...
class NearestNeighborDistanceMetric(object):
    ...
    def __init__(self, metric, matching_threshold, budget=None):
        ...
        self.matching_threshold = matching_threshold  # 匹配阈值，这个在后面的判断cost_matrix中哪些需要去除是需要用到
    ...
    def cost_distance(self, features, targets):
        """Compute distance between features and targets.
        ...
        for i, target in enumerate(targets):
            for j, feature in enumerate(features):
                cost_matrix[i,j] = self._metric(target, feature)
        return cost_matrix


if __name__ == '__main__':
    metric = NearestNeighborDistanceMetric("cosine", 0.2)
    a = np.random.random((2,1,512))
    b = np.random.random((3,1,512))
    cost = metric.cost_distance(a, b)
    print(cost)
    print(cost.shape)
    import ipdb
    ipdb.set_trace()